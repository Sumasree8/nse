...
const signalSchema = new mongoose.Schema({
  ...
  verification: {
    status: { type: String, enum: ['verified', 'corroborated', 'single-source', 'unverified', 'noise'], default: 'unverified', index: true },
    score: { type: Number, min: 0, max: 1, default: 0 },
    credibility: { type: Number, min: 0, max: 1, default: 0 },
    corroborationCount: { type: Number, default: 0 },
    corroboratingSources: [{ domain: String, sourceName: String, url: String, similarity: Number }],
    checkedAt: Date,
  },
